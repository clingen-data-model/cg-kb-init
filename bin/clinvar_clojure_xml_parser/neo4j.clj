(ns clinvar-clojure-xml-parser.neo4j
...
(def neo4j-import-path (str 
                        (or (System/getenv "NEO4J_PATH")  "/Users/nafisakhandaker/clingen/")
                        "/imports/"))
...
(defmacro session
  ...
  clean up after"
  [[s] & body]
  `(let [driver# (GraphDatabase/driver (or (System/getenv "NEO4J_SERVER_PATH") "bolt://localhost:7687") (AuthTokens/basic (or (System/getenv "NEO4J_USER") "neo4j")(or (System/getenv "NEO4J_PASS") "atif5546")))
         ~s (.session driver#)
         result# (do ~@body)]
