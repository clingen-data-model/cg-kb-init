(ns chromatic_data.clinvar-import
  (:require [clojure.java.io :as io]
            [chromatic_data.neo4j :as neo]
            [clojure.edn :as edn]
...
(defn create-constraints
  "Create-constraints"
  [i session]     
  (let [props (walk/stringify-keys (dissoc i :scvid :clinicalassertionid :alleleid :variationid))]
    (.writeTransaction 
     session 
     (neo/tx 
...
(defn import-clinicalassertion
  ...
  [i session]
  (let [props (walk/stringify-keys (dissoc i :scvid :clinicalassertionid :medgencui :variationid))]
    ;(println "props: " props)
    (.writeTransaction 
     session 
     (neo/tx     
       ["FOREACH (x IN CASE WHEN $ClinicalAssertionID IS NULL THEN [] ELSE [1] END |
        ...
        SET a.SCVID= $SCVID,a.SCVVersion=$SCVVersion,a.SubmissionDate=$SubmissionDate,a.DateUpdated=$DateUpdated     
        ...
        MERGE (am)-[:IS_PUBLISHED_IN]->(:Citation {CitationID: $CitationID, CitationSource: $CitationSource}))
        ...
        "DateLastEvaluated" (:datelastevaluated i) "CitationURL" (:citationurl i) "CitationID" (:citationid i) "CitationSource" (:citationsource i) 
    ...
    (.writeTransaction 
     ...
     (neo/tx 
       ["MATCH (a:Assertion {ClinicalAssertionID: $ClinicalAssertionID})
        ...
        {"ClinicalAssertionID" (:clinicalassertionid i) "VariationID" (:variationid i) "VariationName" (:variationname i) "VariationType" (:variationtype i) "props" props}]))))
    ...
    (.writeTransaction 
     ...
     (neo/tx 
       ["MATCH (v:Variation {VariationID: $VariationID})
        ...
        { "AlleleID" (:alleleid i) "AlleleName" (:allelename i) "AlleleType" (:alleletype i) "VariationID" (:variationid i) "props" props}]))))
