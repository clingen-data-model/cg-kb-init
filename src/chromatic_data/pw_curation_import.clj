...
(defn import-gene-disease
  ...
  (let [perm-id (first record)
        ...
        date (curation "dateISO8601")]
    (.run session "match (a:Assertion {perm_id: {id}}), (i:RDFClass {iri: {interp}}) merge (a)-[:has_predicate]->(i) set a.score_string = {scorejson} set a.date = {date}"
          {"interp" interp-iri, "scorejson" score-json,
